...
export const FactureAchatList = () => {
  ...
  const loadInvoices = async () => {
    ...
    try {
      ...
      showSnackbar("Erreur lors du chargement des factures d'achat", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddInvoice = async () => {
    if (!newInvoice.devisAchatId) {
      showSnackbar("Veuillez sélectionner un devis d'achat.", "warning");
      return;
    }
    ...
    try {
      ...
      showSnackbar("✅ Facture d'achat ajoutée avec succès !");
    } catch (error) {
      console.error("Erreur ajout facture d'achat", error);
      showSnackbar("❌ Erreur lors de l'ajout de la facture d'achat", "error");
    } finally {
      setAddLoading(false);
    }
  };
  ...
  const handleEditInvoice = async () => {
    ...
    try {
      ...
      setEditFile(null);
      showSnackbar("✅ Facture d'achat modifiée avec succès !");
    } catch (error) {
      console.error("Erreur modification facture d'achat", error);
      showSnackbar("❌ Erreur lors de la modification de la facture d'achat", "error");
    } finally {
  ...
  const handleDeleteInvoice = async () => {
    ...
    try {
      ...
      showSnackbar("✅ Facture d'achat supprimée avec succès !");
    } catch (error) {
      console.error("Erreur suppression facture d'achat", error);
      showSnackbar("❌ Erreur lors de la suppression de la facture d'achat", "error");
    } finally {
  ...
  const handleViewPdf = useCallback(
    async (item) => {
      if (!item.pdfUrl) {
        showSnackbar("Aucun PDF attaché à cette facture d'achat.", "info");
        return;
      }
      try {
    ...
    <Box>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Gérez toutes vos factures d'achat en un seul endroit
          </Typography>
        </Box>
      </Box>
                ...
                <TableRow>
                  <TableCell colSpan={7} align="center" sx={{ py: 8 }}>
                    ...
                    <Typography color="text.secondary" variant="body2">
                      Commencez par créer une nouvelle facture d'achat
                    </Typography>
                    <Button
                      variant="contained"
                      startIcon={<AddIcon />}
      ...
      >
        <DialogTitle>
          ...
          >
            <Typography variant="h6">Nouvelle facture d'achat</Typography>
                ...
                >
                  {devisAchats.map((d) => (
                    <MenuItem key={d.id} value={d.id}>
                      {d.number} — {d.supplierName || "Fournisseur"}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
      ...
      >
        <DialogTitle>
          ...
          >
            <Typography variant="h6">Modifier la facture d'achat</Typography>
