...
export const BlList = () => {
    ...
    const filteredBl = blList.filter(d =>
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            let response;
            if (isEdit) {
                const itemToEdit = blList.find(d => d.number === formData.number);
                if (itemToEdit) {
                    response = await api.put(`/bl/${itemToEdit.id}`, data);
                }
            } else {
                response = await api.post('/bl', data);
            }
            ...
            alert('Erreur lors de la sauvegarde du BL');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (item) => {
        if (window.confirm(`Êtes-vous sûr de vouloir supprimer le BL ${item.number} ?`)) {
            try {
                await api.delete(`/bl/${item.id}`);
                await loadBl();
            } catch (error) {
    ...
    return (
        <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                <Typography variant="h4" fontWeight={700}>Bons de Livraison</Typography>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
                <DialogTitle>{isEdit ? 'Modifier' : 'Nouveau'} BL</DialogTitle>
                <DialogContent>
                    ...
                    <>
                        ...
                        <DialogContent>
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <Typography variant="body2" color="text.secondary">BDC source</Typography>
                                    <Typography>{selectedItem.bdcNumber || selectedItem.bdc?.number}</Typography>
                                </Grid>
                                <Grid item xs={12} sm={6}>
