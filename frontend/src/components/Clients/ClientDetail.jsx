...
export const ClientDetail = ({ open, client, onClose }) => {
    ...
    const clientDevis = devis.filter(d => d.clientId === client.id || d.client?.id === client.id);
    const clientBDC = bdc.filter(b => clientDevis.some(d => d.id === b.devisId || d.id === b.devis?.id));
    const clientBL = bl.filter(b => clientBDC.some(bdc2 => bdc2.id === b.bdcId || bdc2.id === b.bdc?.id));
    const clientAttachments = attachements.filter(a => clientBL.some(bl2 => bl2.id === a.blId || bl2.id === a.bl?.id));
    const clientFactures = factures.filter(f => clientAttachments.some(a2 => a2.id === f.attachementId || a2.id === f.attachement?.id));
    const clientPaiements = paiements.filter(p => clientFactures.some(f => f.id === p.factureId || f.id === p.facture?.id));
                ...
                <Box sx={{ mt: 2 }}>
                                        ...
                                        <TableBody>
                                            {history.map((item, idx) => (
                                                <TableRow key={idx}>
                                                    ...
                                                    <TableCell>{item.number || item.reference || '-'}</TableCell>
                                                    <TableCell>
                                                    ...
                                                    <TableCell>
                                                        {(item.pdfUrl || item.fileName) && (
                                                            <IconButton
                                                                size="small"
                                                                onClick={() => alert(`Aperçu de ${item.fileName || 'document.pdf'}`)}
                                                            >
                    ...
                    {tabValue === 2 && client.pdfUrl && (
                        <Box sx={{ p: 2, textAlign: 'center' }}>
                            ...
                            <Button variant="outlined" sx={{ mt: 2 }} onClick={() => alert('Téléchargement simulé')}>
                                Télécharger
                            </Button>
                        </Box>
                    )}
