...
const MODE_CONFIG = {
  cash: { label: "Espèces", icon: "💰", color: "#4caf50", bg: "#e8f5e9" },
  check: { label: "Chèque", icon: "📝", color: "#2196f3", bg: "#e3f2fd" },
  transfer: { label: "Virement", icon: "🏦", color: "#9c27b0", bg: "#f3e5f5" },
  card: { label: "Carte bancaire", icon: "💳", color: "#ff9800", bg: "#fff3e0" },
};

const getModeLabel = (mode) => MODE_CONFIG[mode]?.label || mode;
const getModeColor = (mode) => MODE_CONFIG[mode]?.color || "#9e9e9e";
const getModeBg = (mode) => MODE_CONFIG[mode]?.bg || "#f5f5f5";
const getModeIcon = (mode) => MODE_CONFIG[mode]?.icon || "💳";
...
// Composant de mode de paiement
const ModeChip = ({ mode }) => (
  <Chip
    label={`${getModeIcon(mode)} ${getModeLabel(mode)}`}
    size="small"
    sx={{
      bgcolor: getModeBg(mode),
...
export const PaymentsList = () => {
  ...
  }, [payments, searchTerm, dateDebut, dateFin]);
  ...
  const handleAddPayment = async () => {
    if (!newPayment.invoiceId) {
      showSnackbar("Veuillez sélectionner une facture.", "warning");
      return;
    }
    ...
    try {
      ...
      const data = {
        ...
        factureNumber: selectedInvoice?.number || "",
      };
      ...
      resetNewPaymentForm();
      showSnackbar("✅ Paiement ajouté avec succès !");
    } catch (error) {
      console.error("Erreur ajout paiement", error);
      showSnackbar("❌ Erreur lors de l'ajout du paiement", "error");
    } finally {
      setAddLoading(false);
    }
  };
  ...
  // ============================================================
  const handleOpenEdit = useCallback((paymentItem) => {
  ...
  const handleEditPayment = async () => {
    ...
    try {
      ...
      setEditFile(null);
      showSnackbar("✅ Paiement modifié avec succès !");
    } catch (error) {
      console.error("Erreur modification paiement", error);
      showSnackbar("❌ Erreur lors de la modification du paiement", "error");
    } finally {
  ...
  // ============================================================
  const handleOpenDelete = useCallback((paymentItem) => {
  ...
  const handleDeletePayment = async () => {
    ...
    try {
      ...
      showSnackbar("✅ Paiement supprimé avec succès !");
    } catch (error) {
      console.error("Erreur suppression paiement", error);
      showSnackbar("❌ Erreur lors de la suppression du paiement", "error");
    } finally {
  ...
  // VOIR LES DÉTAILS
  // ============================================================
  const handleViewDetail = useCallback((paymentItem) => {
  ...
  const handleViewPdf = useCallback(
    async (paymentItem) => {
      if (!paymentItem.pdfUrl) {
        showSnackbar("Aucun PDF attaché à ce paiement.", "info");
        return;
      }
    ...
    <Box>
      ...
      >
        <Box>
          ...
          >
            ...
            Encaissements
            <Chip
              label={`${filteredPayments.length} paiements`}
              size="small"
              color="primary"
              variant="outlined"
            />
          ...
          <Typography variant="body2" color="text.secondary">
            Gérez tous vos encaissements en un seul endroit
          </Typography>
        </Box>
      </Box>
          ...
          <Table>
            <TableHead sx={{ bgcolor: "grey.50" }}>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 700 }}>Facture</TableCell>
            ...
            <TableBody>
              {paginatedPayments.length === 0 ? (
                ...
                paginatedPayments.map((d) => (
                  <TableRow key={d.id} hover>
                    <TableCell>
                      <Typography fontWeight={600}>{d.reference}</Typography>
                    </TableCell>
                    <TableCell>
                      <Box
      ...
      >
        <DialogTitle>
          ...
          >
            <Typography variant="h6">Nouveau paiement</Typography>
        ...
        <DialogActions sx={{ p: 3, pt: 0 }}>
          ...
          >
            {addLoading ? "Ajout en cours..." : "Ajouter le paiement"}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        <DialogTitle>
          ...
          >
            <Typography variant="h6">Modifier le paiement</Typography>
        ...
        {selectedPayment && (
          <>
            <DialogTitle>
              ...
              >
                <Box>
                  <Typography variant="h6">{selectedPayment.reference}</Typography>
                  <Typography variant="body2" color="text.secondary">
                    Détails du paiement
                  </Typography>
                </Box>
            ...
            <DialogContent sx={{ pt: 3 }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <ModeChip mode={selectedPayment.mode} />
                </Grid>
                <Grid item xs={12}>
                  <Divider sx={{ my: 1 }} />
                  <Typography variant="body2" color="text.secondary" gutterBottom>
                  ...
                  <Typography variant="body1">
                    {selectedPayment.bankReference || "Aucune référence"}
                  </Typography>
                </Grid>
                <Grid item xs={12}>
                  <Paper
                ...
                <Grid item xs={12}>
                  ...
                  <Typography variant="body1">
                    {selectedPayment.comments || "Aucun commentaire"}
                  </Typography>
                </Grid>
